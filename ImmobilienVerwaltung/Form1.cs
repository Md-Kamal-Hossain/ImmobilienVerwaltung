...
namespace ImmobilienVerwaltung
{
    public partial class Form1 : Form
    {
...

        {
            ...
            double wohnflache;
            double kellerflache;
            if (!int.TryParse(textBox_baujahr.Text, out bauja) || !double.TryParse(textBox_GründstückSize.Text, out grunflasche) || !double.TryParse(textBox_WohnfläscheSize.Text, out wohnflache)|| !double.TryParse(textBox_Kellerfläschesize.Text, out kellerflache))
            {
                MessageBox.Show("Please put only integer value for Year;and Decimal or Integer for Gründflasche, Wohnflasche and Kellerflasche!");
            }
           else if (string.IsNullOrEmpty(textBox_baujahr.Text) && string.IsNullOrEmpty(textBox_GründstückSize.Text)
                && string.IsNullOrEmpty(textBox_WohnfläscheSize.Text) && string.IsNullOrEmpty(textBox_Kellerfläschesize.Text) && string.IsNullOrEmpty(textBox_StraßeName.Text) && string.IsNullOrEmpty(textBox_HausNr.Text))
            {
